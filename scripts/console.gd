...
func help() -> void:
	...
	printd("clear() - Clears the text in the console.")
	printd("printd(data, color: Color = Color.YELLOW) - Prints data to the console with an optional color.")
	printd("get_cid() - Displays the current control ID, player ID, and the number of players in the world.")
	printd("glog() - Reads and prints lines from the log file if it exists.")
	printd("tp(x: int = 1, y: int = 1, z: int = 1, pl_id: int = Event.mpp_index) - Teleports a player to specified coordinates.")
	...
	printd("give(item_id, amount: int = 1, pl_id: int = Event.mpp_index) - Gives an item to a player.")
	printd("set_event(event_id: String) - Sets the current event by ID.")
...
func set_event(event_id:String):
